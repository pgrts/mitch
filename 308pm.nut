...
flagspawn.CFG <- {
    ...
    SPAWNER_PROP_BLU = "blu_flagspawner_prop",// prop_dynamic (meter at spawn)
...
flagspawn._ApplyVisualState <- function(pkg) {
    ...
    if (FS_IsValid(owner) && FS_IsPlayer(owner)) {
        ...
        // Hide flag draw (engine uses it for inventory, we show prop)
        flagspawn._EntFire(pkg.flag, "DisableDraw", "", 0.0, null, null);
        ...
        // Show flag draw
        flagspawn._EntFire(pkg.flag, "EnableDraw", "", 0.0, null, null);
...
flagspawn._UpdateUI <- function() {
    ...
    if (FS_IsValid(txt)) flagspawn._EntFire(txt, "SetMessage", slotsLeft.tostring(), 0.0, null, null);
    ...
    // Spawner meter shows pool (clamped 0..100)
...
function FS_OnFlagEvent() {
    ...
    try { idx = evt.index; } catch (e1) { idx = -1; }

    // Find pkg by entindex
    local foundSuf = null;
    foreach (suf, pkg in flagspawn.State.Flags) {
        if (!pkg || !FS_IsValid(pkg.flag)) continue;
        if (pkg.flag.entindex() == idx) { foundSuf = suf; break; }
    }

    // 1: Pickup, 2: Capture, 4: Drop, 5: Return
    if (type == 1 || type == 4) {
        if (foundSuf) {
            local pkg2 = flagspawn.State.Flags[foundSuf];
            pkg2.retry = flagspawn.CFG.RETRY_COUNT;
            flagspawn._BumpGlowUntil(pkg2, flagspawn.CFG.EVENT_GLOW_DURATION);
        }
    }

    // Refund on Capture/Return (kill entity to free slots)
    if (type == 2 || type == 5) {
        if (foundSuf) {
            local pkg3 = flagspawn.State.Flags[foundSuf];

            local refund = pkg3.pointValue;
            if (refund > flagspawn.CFG.VALUE_CAP) refund = flagspawn.CFG.VALUE_CAP;
            if (refund < 0) refund = 0;

            flagspawn.State.PoolBlu += refund;

            // Kill everything; flag would otherwise respawn at base
            if (FS_IsValid(pkg3.flag)) pkg3.flag.Kill();
            if (FS_IsValid(pkg3.prop)) pkg3.prop.Kill();
            if (FS_IsValid(pkg3.glow)) pkg3.glow.Kill();
            if (FS_IsValid(pkg3.propGlow)) pkg3.propGlow.Kill();
            if (FS_IsValid(pkg3.lock)) pkg3.lock.Kill();
            if (FS_IsValid(pkg3.lmm)) pkg3.lmm.Kill();

            delete flagspawn.State.Flags[foundSuf];

            flagspawn._UpdateUI();
            flagspawn.State.NextTopKTime = 0.0;

            FS_Log("Refund (type=" + type + ") +" + refund + " pool=" + flagspawn.State.PoolBlu);
        }
    }

    // Immediate pulse catch
    flagspawn._Pulse();
}
...
// Host entity resolver
// - When run via a logic_script (vscripts), `self` exists.
// - When run via `script_execute`, `self` does NOT exist.
// We always schedule Think on the logic_script named CFG.SCRIPTER_NAME.
flagspawn._GetHost <- function() {
    // Prefer self if present
    try {
        if (("self" in getroottable()) && self && self.IsValid()) return self;
    } catch(e) {}

    // Prefer named logic_script
    local h = Entities.FindByName(null, flagspawn.CFG.SCRIPTER_NAME);
    if (h && h.IsValid()) return h;

    // Last resort: any logic_script
    h = Entities.FindByClassname(null, "logic_script");
    if (h && h.IsValid()) return h;
...
flagspawn.Init <- function() {
    ...
    local host = flagspawn._GetHost();
    if (!host) {
        // We can still run manual calls, but cannot schedule Think.
        FS_Log("[ERROR] Init: no host entity (self/scripter). Think not scheduled.");
        flagspawn._UpdateUI();
        return;
    }
