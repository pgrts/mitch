// ============================================================
// flagspawn_agentic_tf2_fixed.nut
// TF2 VScript–compatible Flagspawn script (syntax-safe)
// ============================================================
...
printl("[flagspawn] AGENTIC TF2 FIXED BUILD LOADED @ t=" + Time());
...
// ============================================================
::flagspawn.flagPools <- {};
::flagspawn.flagPools[::TEAM_RED] <- [];
::flagspawn.flagPools[::TEAM_BLU] <- [];
...
function TeamName(t)
{
    if (t == ::TEAM_RED) return "RED";
    if (t == ::TEAM_BLU) return "BLU";
    return "TEAM_" + t;
}

// ============================================================
// PLAYER STATE (TF2-SAFE)
// ============================================================
...
function ::flagspawn.InitPools()
{
    ...
    local flag = null;
    while ((flag = Entities.FindByClassname(flag, "item_teamflag")) != null)
    {
...
function ::flagspawn.OnSpawnerTouch(player, baseTeam)
{
    ...
    if (ps.hasFlag)
    {
        FS_Log("DENY: already carrying flag");
        return;
    }
    ...
    FS_Log("GRANT: " + TeamName(playerTeam) +
           " stole " + TeamName(baseTeam) +
           " flag eidx=" + flag.entindex());
}
