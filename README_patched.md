...
**Important PD quirk:** in stock **Player Destruction**, `item_teamflag` **OnPickup / OnPickup1** does **not** fire when the player is already carrying points (i.e., when a pickup would *merge* into an existing stack).

**Pivot:** we treat `item_teamflag` outputs as *world-entity lifecycle hooks* (spawned pool flags), but we detect **MERGE** from the **player’s carried total** and/or **`teamplay_flag_event`** (PICKEDUP while already carrying).

**Signals we use:**
- **World flag outputs (VMF-wired):**
  - `OnPickup1` → `::flagspawn.OnPoolFlagPickup(caller, activator)`
  - `OnDrop1` → `::flagspawn.OnPoolFlagDrop(caller, activator)`
  - `OnReturn` → `::flagspawn.OnPoolFlagReturn(caller)`
  - `OnCapture1` → `::flagspawn.OnPoolFlagCapture(caller, activator)`
- **Merge detection (script):**
  - Listen to `teamplay_flag_event` and treat **PICKEDUP** as:
    - **Pickup-start** if the carrier previously had 0
    - **Merge** if the carrier previously had >0
  - (Optionally) also compare `lastCarry[player]` → `curCarry[player]` each tick to catch edge cases.

This keeps your **output-driven pipeline** (easy VMF wiring) while making merges reliable in PD.
